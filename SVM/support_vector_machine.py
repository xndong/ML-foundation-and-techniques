...
def innerL(i, oS):  # 输入参数i和所有参数数据
    ...
    if ((oS.labelMat[i]*Ei < -oS.tol) and (oS.alphas[i] < oS.C)) or ((oS.labelMat[i]*Ei > oS.tol) and (oS.alphas[i] > 0)):
        ...
        eta = 2.0 * oS.K[i, j] - oS.K[i, i] - \
            oS.K[j, j]  # 参考《统计学习方法》p127公式7.107
        if eta >= 0:
            print("eta>=0")
            return 0
        ...
        oS.alphas[i] += oS.labelMat[j]*oS.labelMat[i] * \
            (alphaJold - oS.alphas[j])  # 参考《统计学习方法》p127公式7.109
        updateEk(oS, i)  # 更新数据
...
def testRbf(data_train, data_test):
    ...
    errorCount = 0
    for i in range(m):
        kernelEval = kernelTrans(
            sVs, datMat[i, :], ('rbf', 1.3))  # 将支持向量转化为核函数
...
def main():
    filename_traindata = r'C:\Users\DongXiaoning\Downloads\Algorithms in Machine Learning Foundation and Techniques\SVM\traindata.txt'
    filename_testdata = r'C:\Users\DongXiaoning\Downloads\Algorithms in Machine Learning Foundation and Techniques\SVM\testdata.txt'
