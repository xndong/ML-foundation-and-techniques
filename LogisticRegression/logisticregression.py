...
#using stochastic gradient descent as solver (of optimization problem), vector/matrixwise
...
#using batch gradient descent as solver (of optimization problem), vector/matrixwise
def batchGradientDescent(trainData,trainLabel,w,alpha,batchSize,maxIteration):
    m, n = np.shape(trainData)
    if m % batchSize == 0:
    ...
    for it in range(maxIteration):
        ...
        if currentBatchIndex==totalBatch-1:
            #the last batch
            trainData=trainData[currentBatchIndex:currentBatchIndex+batchSize-1,:]
...
#using stochastic gradient descent as solver (of optimization problem), elementwise
